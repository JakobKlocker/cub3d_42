...
void	rayCasting(t_mapInfo *mapInfo)
{
	...
	while (x < mapInfo->screen.width)
	{
		...
		drawLine(mapInfo, x, mapInfo->rayInfo.drawStart,
				mapInfo->rayInfo.drawEnd);
		x++;
	}
...
void	calcStep(t_rayInfo *rayInfo)
{
	...
	{
		rayInfo->stepX = -1;
		rayInfo->sideDistX = (rayInfo->posX - rayInfo->mapX)
			* rayInfo->deltaDistX;
	}
	else
	{
		rayInfo->stepX = 1;
		rayInfo->sideDistX = (rayInfo->mapX + 1.0 - rayInfo->posX)
			* rayInfo->deltaDistX;
	}
	if (rayInfo->rayDirY < 0)
	{
		rayInfo->stepY = -1;
		rayInfo->sideDistY = (rayInfo->posY - rayInfo->mapY)
			* rayInfo->deltaDistY;
	}
	else
	{
		rayInfo->stepY = 1;
		rayInfo->sideDistY = (rayInfo->mapY + 1.0 - rayInfo->posY)
			* rayInfo->deltaDistY;
	}
}
