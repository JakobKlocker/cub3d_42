...
int	move_hook(int key, t_mapInfo *mapInfo)
{
	...
	if (key == XK_Left || key == XK_a)
	{
		...
		rayInfo->dirY = rayInfo->oldDirX * sin(-0.1) + rayInfo->dirY
			* cos(-0.1);
		rayInfo->oldPlaneX = rayInfo->planeX;
		rayInfo->planeX = rayInfo->planeX * cos(-0.1) - rayInfo->planeY
			* sin(-0.1);
		rayInfo->planeY = rayInfo->oldPlaneX * sin(-0.1) + rayInfo->planeY
			* cos(-0.1);
		rayCasting(mapInfo);
		ft_minimapBG(mapInfo, &mapInfo->minimap);
	}
